...
import { Plus, Search, Grid, List, Package, Globe, Check, Download, Loader2, Filter, Trash2, Settings as SettingsIcon, Palette } from "lucide-react";
...
export default function SkillsPage() {
  ...
  const fetchTokenAndRepo = useCallback(async () => {
    // Registry is now managed explicitly. We just keep defaults.
    setRegistryRepo("deepsuthar496/BraneRegistry");
  }, []);
  ...
  const fetchCategorySkills = useCallback(async (categoryId: string) => {
    setLoading(true);
    try {
      const data = await window.electronAPI.fetchRegistryData<SkillEntry[] | { skills: SkillEntry[] }>(
        registryUrls.skillCategory(categoryId)
      );
      const skills = Array.isArray(data) ? data : (data && (data as any).skills ? (data as any).skills : []);
                ...
                <div className="flex-1 overflow-y-auto p-2">
                  ...
                  <nav className="space-y-0.5">
                    {registryIndex?.categories.skills.map((cat) => {
                      const Icon = cat.icon === "Settings" ? SettingsIcon : (cat.icon === "Palette" ? Palette : Package);
                      return (
                        <button
                          key={cat.id}
                        ...
                        >
                          <Icon className={cn("size-3.5 transition-colors", activeCategory === cat.id ? "text-primary" : "text-txt-4 group-hover:text-foreground")} />
                          <span className="flex-1 text-left">{cat.label}</span>
                          <span className={cn(
                            "text-[10px] font-mono px-1.5 py-0.5 rounded bg-background/50 border border-border/50",
                            activeCategory === cat.id ? "text-primary/70 border-primary/20" : "text-txt-4"
                          )}>{cat.count}</span>
                        </button>
                      );
                    })}

...
// Mock Data Removed. Fetching from Registry now.
import { getRegistryUrls } from "@/lib/registry";

export default function StorePage() {
  const [storeAgents, setStoreAgents] = useState<any[]>([]);
  ...
  const fetchStoreIndex = useCallback(async () => {
    if (typeof window !== "undefined" && window.electronAPI) {
      try {
        setLoading(true);
        const urls = getRegistryUrls("deepsuthar496/BraneRegistry");
        const data: any = await window.electronAPI.fetchRegistryData(urls.agentstore);
        if (data && data.agents) {
          // Map index agents to basic UI structure
          const mappedAgents = data.agents.map((a: any) => ({
            ...a,
            shortDescription: a.tagline,
            icon: "lucide:terminal", // Simple fallback icon
            rating: "4.9",
            downloads: "N/A"
          }));
          setStoreAgents(mappedAgents);
        }
      } catch (error) {
        console.error("Failed to load agent store index:", error);
      } finally {
        setLoading(false);
      }
    }
  ...
  // Sudden refresh listener
  useEffect(() => {
    if (typeof window === "undefined") return;
    const handleRefresh = () => {
      console.log("[Store] Token updated, refreshing store index...");
      fetchStoreIndex();
    };
  ...
  useEffect(() => {
    async function loadAgentDetails() {
      if (selectedAgent && window.electronAPI) {
        try {
          const urls = getRegistryUrls("deepsuthar496/BraneRegistry");
          const detailUrlPair = urls.agentDetail(selectedAgent.id);
          const data: any = await window.electronAPI.fetchRegistryData(detailUrlPair);
          
          if (data) {
            // Map install options
            const installOptions: any[] = [];
            if (data.install.npm) installOptions.push({ id: "npm", os: "NPM (Global)", icon: "logos:npm-icon", command: data.install.npm, platformMatch: ["any"] });
            if (data.install.pip) installOptions.push({ id: "pip", os: "Python (Pip)", icon: "logos:python", command: data.install.pip, platformMatch: ["any"] });
            if (data.install.brew) installOptions.push({ id: "homebrew", os: "Homebrew", icon: "devicon:homebrew", command: data.install.brew, platformMatch: ["mac"] });
            if (data.install.script_mac_linux) installOptions.push({ id: "script_mac_linux", os: "MacOS / Linux (Script)", icon: "ph:terminal-fill", command: data.install.script_mac_linux, platformMatch: ["mac", "linux"] });
            if (data.install.script_windows) installOptions.push({ id: "script_windows", os: "Windows (Script)", icon: "ph:terminal-fill", command: data.install.script_windows, platformMatch: ["win"] });
            
            data.installOptions = installOptions;
            data.commandName = data.id.split('-')[0]; // Simple guess if not provided
            
            setAgentDetails(data);
            
            // Check if installed
            window.electronAPI.checkCLIInstalled(data.commandName).then(setIsInstalled);
            
            // Fetch about markdown if available
            if (data.aboutUrl) {
              const md = await window.electronAPI.fetchUrl(data.aboutUrl);
              setAboutMarkdown(md);
            }
          }
        } catch (error) {
  ...
  useEffect(() => {
    if (agentDetails && userPlatform !== "unknown") {
      // Find best default installer for current OS
      const bestMatch = agentDetails.installOptions.find((opt: any) =>
        opt.platformMatch.includes(userPlatform) || opt.platformMatch.includes("any")
      );
      if (bestMatch) {
        setSelectedInstallId(bestMatch.id);
      } else if (agentDetails.installOptions.length > 0) {
        setSelectedInstallId(agentDetails.installOptions[0].id);
      }
    }
  }, [agentDetails, userPlatform]);
  ...
  const handleInstall = async () => {
    if (!selectedAgent || !agentDetails || !selectedInstallOption) return;
  ...
  const handleAbortInstall = async () => {
    if (!selectedAgent || !window.electronAPI) return;
    try {
      await window.electronAPI.abortInstall(selectedAgent.id);
      setInstallLogs(prev => [...prev, "\n[System] Installation aborted by user."]);
      setIsInstalling(false);
    } catch (err: any) {
  ...
  const selectedInstallOption = agentDetails?.installOptions?.find((opt: any) => opt.id === selectedInstallId) || agentDetails?.installOptions?.[0];
    ...
    <div className="flex flex-col h-screen">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 flex flex-col overflow-hidden bg-background">
          ...
          <div className="flex-1 overflow-y-auto bg-background px-8 py-6 no-scrollbar">
            {!selectedAgent ? (
              // --- LIST VIEW ---
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {isInstalling && (
                            <Button
                              ...
                              className="h-12 px-6 rounded-xl border border-red-500/20 text-red-400 hover:bg-red-500/10 font-bold"
                            >
                              Cancel
                            </Button>
                          )}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 xl:gap-20">
                  ...
                  <div className="lg:col-span-2 space-y-10">
                    ...
                    <section>
                      ...
                      {aboutMarkdown ? (
                        <div className="prose prose-invert max-w-none prose-sm opacity-90">
                           <Markdown content={aboutMarkdown} />
                        </div>
                      ) : (
                        <p className="text-[16px] text-txt-2 leading-relaxed opacity-90">
                          {selectedAgent.description || agentDetails?.description}
                        </p>
                      )}
                    </section>
                    ...
                    <section className="bg-surface-1/50 rounded-3xl p-6 border border-border/40 max-w-2xl">
                      ...
                      <div className="grid grid-cols-2 gap-y-4">
                        <div>
                          <span className="text-[12px] text-txt-4 block mb-1">Package Name</span>
                          <span className="text-[14px] font-semibold text-foreground font-mono">{agentDetails?.packageName || selectedAgent.id}</span>
                        </div>
                        <div>
                          <span className="text-[12px] text-txt-4 block mb-1">Verify Command</span>
                          <span className="text-[14px] font-semibold text-foreground font-mono">{agentDetails?.verifyCommand || `${selectedAgent.id} --version`}</span>
                        </div>
                        <div>
                          <span className="text-[12px] text-txt-4 block mb-1">Category</span>
                          <span className="text-[14px] font-semibold text-foreground capitalize">{selectedAgent.category}</span>
                        </div>
                        <div>
                          <span className="text-[12px] text-txt-4 block mb-1">License</span>
                          <span className="text-[14px] font-semibold text-foreground">Open Source</span>
                        </div>
                      </div>
                    </section>
                  </div>
                      ...
                      <div className="bg-surface-1 border border-border/60 rounded-3xl p-6 shadow-lg">
                        ...
                        <div className="flex flex-col gap-2">
                          {!agentDetails ? (
                            ...
                            agentDetails.installOptions?.map((opt: any) => {
